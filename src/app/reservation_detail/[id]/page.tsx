...
export default function ReservationDetailPage() {
  ...
  useEffect(() => {
    ...
    const initializeLiff = async () => {
      try {
        ...
        setLiffError(
          "LINE連携に失敗しました。LINEアプリ内のブラウザで開いてください。"
        );
        setLoading(false);
      }
    };
  ...
  const isFull =
    reservation.maxMembers &&
    reservation.memberNames.length >= reservation.maxMembers;
  const isAlreadyJoined = lineProfile
    ? reservation.memberNames.includes(lineProfile.displayName)
    : false;

  return (
    <Container maxWidth="sm" sx={{ py: 2 }}>
      <Stack direction="row" alignItems="center" mb={2}>
        <IconButton onClick={() => liff.closeWindow()}>
          <ArrowBackIcon />
        </IconButton>
      ...
      <Stack spacing={2}>
        ...
        <Typography variant="h6">
          時間: {reservation.startTime.slice(0, 5)} 〜{" "}
      ...
      <Divider sx={{ my: 3 }} />
      <Typography variant="h6">
        メンバー ({reservation.memberNames.length} /{" "}
        {reservation.maxMembers || "N/A"}人)
      </Typography>
      <List>
