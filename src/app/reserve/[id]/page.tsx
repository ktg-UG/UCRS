    ...
    if (id) {
      const fetchDetail = async () => {
        ...
        try {
          ...
          setFormData({
            ...data,
            purpose: data.purpose || '', // purposeがundefinedの場合、空文字を設定
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 2 }}>
      <Stack direction="row" alignItems="center" mb={2}>
        ...
        <Typography variant="h5" sx={{ flexGrow: 1, textAlign: 'center' }}>募集詳細・編集</Typography>
        <Box sx={{ width: 40 }} />
      </Stack>
      <ReservationForm formData={formData} setFormData={setFormData} isPrivate={isPrivate} disabled={!editMode} isEditMode={true} />
      <Stack spacing={2} mt={3}>
        {!editMode ? (
          <Button fullWidth variant="contained" onClick={() => setEditMode(true)}>編集する</Button>
        ) : (
          <>
            ...
            <Button fullWidth variant="outlined" color="secondary" onClick={() => setEditMode(false)}>キャンセル</Button>
          </>
        )}
        <Button fullWidth variant="contained" color="error" onClick={handleDelete} sx={{ mt: 1 }}>この募集を削除する</Button>
      </Stack>
    </Container>
  );
}