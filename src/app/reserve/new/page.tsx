...
function ReserveNewForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (reservationType === 'ボールのみ予約' && (!formData.memberNames[0] || formData.memberNames[0].trim() === '')) {
      alert('代表者名を入力してください。');
      return;
    }
    ...
    const payload = {
      ...
      purpose: reservationType === 'ボールのみ予約' ? 'ボールのみ予約' : formData.purpose,
      maxMembers: reservationType === 'ボールのみ予約' ? 1 : formData.maxMembers,
